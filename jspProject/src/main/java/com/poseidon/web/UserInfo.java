...
public class UserInfo extends HttpServlet {
   ...
   protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      if (session.getAttribute("m_id") != null) {
         ...
         pw.println("아이디 : " + dto.getId() + "<dr>");
         pw.println("암호 : "+ dto.getPasswd() + "<dr>");
         pw.println("이름 : "+ dto.getName()+ "<dr>");
         pw.println("주소 : "+ dto.getAddr()+ "<dr>");
         pw.println("전화번호 : "+ dto.getTel()+ "<dr>");
         pw.println("나이 : "+ dto.getAge()+ "<dr>");
         pw.println("<hr>");
         pw.println("</html>");
         System.out.println("UserInfo -> get");
         RequestDispatcher rd = request.getRequestDispatcher("./userInfo.jsp");
         //주소표시줄을 유지하고 화면만 변경한다.
         //값 가져가
         request.setAttribute("userInfo", dto); //이름, 값
         rd.forward(request, response);//변경 시작
