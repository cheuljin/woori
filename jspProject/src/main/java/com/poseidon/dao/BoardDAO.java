...
public class BoardDAO {
	public List<BoardDTO> boardList(int i) {
		List<BoardDTO> list = new ArrayList<BoardDTO>();
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = "SELECT * FROM may_boardview limit ?, 10"; // view 만들거라서 잠시후 수정
	...
	public void commetWrite(CommentDTO dto) {
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = "INSERT INTO may_comment (b_no, c_comment, m_no) VALUES (?, ?, (SELECT m_no FROM may_member WHERE m_id=?))";
		try {
			con = DBConnection.dbConn();
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, dto.getB_no());
	...
	public void commentUpdate(CommentDTO dto) {
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = "UPDATE may_comment SET c_comment=? WHERE c_no=? AND m_no=(SELECT m_no FROM may_member WHERE m_id=?)";
